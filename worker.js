...
async function resolveMirrorSource(store, source, set) {
  ...
  if (!original || !rewritten || String(original.chatId || original.user)===String(rewritten.chatId || rewritten.user)) return rewritten;
  ...
  const managed=contentChannelRules(store.env)[from]?.targets.includes(to) || await store.get('mirror-pair:'+from+'/'+to);
...
async function fulfillDownload(store, ctx, token, chatId, dl, set) {
  ...
  for (let i = 0; i < sources.length; i++) {
    let src;
    try { src=await resolveMirrorSource(store,sources[i],set); }
    catch(e) { await tgSend(token,chatId,'❌ '+e.message); return; }
    const partKey = JSON.stringify([src.chatId || src.user || '', src.msgId || '', src.fileId || '']);
    if (dl.sentParts.includes(partKey)) continue;
    const r = await deliverMedia(token, chatId, src, cap, {reply_markup:kb || undefined, set:{...set,vaultRewrite:'',vaultRewriteEnv:''}});
    if (!r.ok) {
  ...
  if (tab === 'settings') {
    return '<div class="adm-h">⚙️ تنظیمات سایت</div>' +
      ...
      (data.vaultRewriteFromEnv
        ...
        : '<div class="field"><label>جایگزینی فقط اگر کانال عوض شد — هر خط: قدیم جدید؛ برای مخزن‌های ربات، شناسه عددی هر دو کانال (-100...) را بنویسید</label><textarea id="set-vault" rows="2" placeholder="xvcdn newcdn">' + esc(data.vaultRewrite || '') + '</textarea></div>') +
