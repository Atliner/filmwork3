...
export default {
  async fetch(request, env, ctx) {
    ...
    if (storagePath==='/storage-migration') return new Response(D1_MIGRATION_HTML,{headers:{'content-type':'text/html; charset=utf-8','cache-control':'no-store'}});
...
function adminTabHtml (tab, data, q) {
  ...
  if (tab === 'settings') {
    return '<div class="adm-h">⚙️ تنظیمات سایت</div>' +
      ...
      '<div class="note"><a href="/storage-migration" target="_blank" rel="noopener">انتقال امن دیتابیس به D1</a> — ابتدا راهنما و پشتیبان‌گیری؛ اتصال DB به‌تنهایی کافی نیست.</div>' +
...
<label><input id="safe" type="checkbox"> پشتیبان دارم و تأیید می‌کنم منبع دیگر تغییر نمی‌کند.</label><p><button id="backup">دانلود پشتیبان KV</button><button id="status">بررسی وضعیت</button><button id="start">شروع / ادامه انتقال</button><button id="stop" disabled>توقف پس از این مرحله</button></p>
