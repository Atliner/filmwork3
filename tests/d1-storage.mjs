...
assert.equal((await call('', 'authorize')).status,403);
assert.equal((await call(await app.signToken({u:'reader'},'test-secret'),'authorize')).status,403);
const tool=await (await call(token,'authorize')).json();assert(tool.html.includes('id="backup"'));
const gate=await app.default.fetch(new Request('https://test/storage-migration'),env,{});
assert.equal(gate.headers.get('x-frame-options'),'DENY');
const gateHtml=await gate.text();assert(!gateHtml.includes('id="backup"'));
new vm.Script(gateHtml.match(/<script>([\s\S]*?)<\/script>/)[1]);
...
const withoutKV={...active,KV:undefined,CONTENT_BOT_TOKEN:'fake',CONTENT_WEBHOOK_SECRET:'test-secret',EDITOR:{}};
assert.equal((await app.default.fetch(new Request('https://test/'),withoutKV,{})).status,200);
assert.equal((await app.default.fetch(new Request('https://test/api/tg/content-webhook',{method:'POST',headers:{'x-telegram-bot-api-secret-token':'test-secret'},body:'{}'}),withoutKV,{})).status,200);
assert.equal((await app.default.fetch(new Request('https://test/api/admin/maintenance',{method:'POST',body:JSON.stringify({prefix:'mirror:'})}),withoutKV,{})).status,403);
const maintenanceRequest=()=>new Request('https://test/api/admin/maintenance',{method:'POST',headers:{authorization:'Bearer '+token},body:JSON.stringify({prefix:'mirror:'})});
assert.equal((await app.default.fetch(maintenanceRequest(),withoutKV,{})).status,200);
await store.set('u:admin',{username:'admin',role:'free',tgId:'42'});
assert.equal((await app.default.fetch(maintenanceRequest(),withoutKV,{})).status,403);
assert.equal((await app.default.fetch(new Request('https://test/api/admin/storage/d1',{method:'POST',headers:{authorization:'Bearer '+token},body:JSON.stringify({action:'authorize'})}),withoutKV,{})).status,403);
assert.equal(kvWrites,0);
