...
try {
  ...
  assert.equal(ctx.entries.get('draft').files[0].title,'Rubixfa | 720p | 0 MB');
  assert.equal(ctx.entries.get('draft').files[0].sizeBytes,123);
  ...
  assert.equal(data.get('it:i_abc').seasons[0].episodes.find(e=>e.n===4).variants.dub['720'].files[0].title,'Rubixfa | 720p | 0 MB');
