...
try {
 ...
 assert(![...data.keys()].some(k=>k.startsWith('mirror:') || k.startsWith('mirror-pair:')));
 await state.storage.put('album-seen:album-a',Date.now()-3000);
 const put=state.storage.put;let failMapping=true;
 state.storage.put=async(k,v)=>{if(k==='mirror-file:3' && failMapping){failMapping=false;throw new Error('DO write unavailable');}return put(k,v);};
 await tick();
 assert.equal(tgCalls.filter(c=>c.method==='copyMessages'&&c.body.chat_id===targetA).length,1);
 // Recreate the object after partial DO progress: replay saved response, not copy.
 sessions.set('channel:'+channel,new app.EditorSession(state,env));
 await tick();state.storage.put=put;
 assert.equal(tgCalls.filter(c=>c.method==='copyMessages'&&c.body.chat_id===targetA).length,1);
 ...
 assert.equal(resolved.msgId,String(state.values.get('mirror-file:2')[targetA].msgId));
 ...
 assert(!state.values.get('mirror-file:4')?.[targetA]);
 ...
 const savedPut=kv.put; kv.put=async()=>{throw new Error('KV quota exhausted');};
 short=false;
 await session.fetch(new Request('https://internal/channel-relay',{method:'POST',body:JSON.stringify({channel,actorId:'42',files:[{chatId:channel,msgId:90}]})}));
 await tick(); kv.put=savedPut;
 assert(state.values.get('mirror-file:90')[targetA]);
 assert(![...data.keys()].some(k=>/^mirror[:\-]/.test(k)),'relay must not write mirror data to KV');
} finally {globalThis.fetch=oldFetch;}
console.log('PASS: album preservation, quiet-window collection, actual destination ID mapping, restart/partial DO recovery, zero mirror KV writes, safe incomplete-copy handling and managed-vault fallback.');
